...
`author_original_language_id` is a foreign key to the [`language` table](data-model.md#language), whose primary key is the ISO 639-1 code itself (e.g. `it`, `ru`) — the author files reference the codes directly. The table is seeded from `assets/db/sql/language/language_0001.sql` (the full ISO 639-1 set, generated by `assets/db/tools/iso_seed.py`); load it before any author file.
