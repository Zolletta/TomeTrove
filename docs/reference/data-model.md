...
Reference table of ISO 3166-1 countries, used to pre-fill the country selector (`user_country`) and to validate `store_ships_to` entries.

Table: `country`

| Field           | Type        | Notes                                                                        |
|-----------------|-------------|------------------------------------------------------------------------------|
| country_id      | PK (string) | ISO 3166-1 alpha-2 code (e.g. `IT`, `DE`) — the natural key, no surrogate id |
| country_name_en | string      | English short name (e.g. "Italy")                                            |

**Pre-loaded**: all officially assigned ISO 3166-1 alpha-2 codes, seeded from `assets/db/sql/country/country_0001.sql` (generated by `assets/db/tools/iso_seed.py`). Codes and names are downloaded from the [Debian iso-codes project](https://salsa.debian.org/iso-codes-team/iso-codes) (`iso_3166-1.json`).

## Currency

Reference table of ISO 4217 currencies, used to pre-fill the currency selector (`user_currency`) and to validate `store_currencies` and price quote currencies.
...
**Pre-loaded**: all active ISO 4217 currencies (funds excluded), seeded from `assets/db/sql/currency/currency_0001.sql` (generated by `assets/db/tools/iso_seed.py`). Codes, names and minor units are downloaded from the [SIX Financial `list-one.xml`](https://www.six-group.com/en/products-services/financial-information/data-standards.html), published by SIX as the ISO 4217 maintenance agency.
