...
def build_row(qid: str, record: dict, code: str) -> dict | None:
    ...
    return {
        ...
        "latin": parsed.latin,
        "original": original,
        "language_id": lang_registry.language_id(original_language) if original_language else None,
        "aliases": aliases,
        "wikidata": qid,
...
def write_sql(rows: list[dict], code: str, name_en: str, out_dir: Path) -> list[Path]:
    ...
    for number, part in enumerate(parts, start=1):
        ...
        with path.open("w", encoding="utf-8") as fh:
            ...
            fh.write("-- Load 0000_language_seed_tmp.sql first: author_original_language_id references it.\n")
...
def write_language_seed(out_dir: Path) -> Path:
    path = out_dir / "0000_language_seed_tmp.sql"
    with path.open("w", encoding="utf-8") as fh:
        fh.write("-- TomeTrove temporary language seed.\n")
        fh.write("-- Generated by assets/db/tools/harvest.py. Do not hand-edit.\n")
        fh.write("-- Temporary: dropped once the production language seed lands, at which\n")
        fh.write("-- point author_original_language_id is re-resolved by language_code.\n")
        fh.write("-- The ids are fixed in assets/db/tools/languages.py and referenced\n")
        fh.write("-- verbatim by the author files, so a regenerated seed keeps the keys.\n\n")
        fh.write("SET NAMES utf8mb4;\n\n")
        fh.write("INSERT INTO language (language_id, language_code, language_name_en, language_name_native) VALUES\n")
        rows = [
            f"  ({lid}, {sql_literal(code)}, {sql_literal(name_en)}, {sql_literal(name_native)})"
            for lid, code, _qid, name_en, name_native in lang_registry.LANGUAGES
        ]
        fh.write(",\n".join(rows) + ";\n")
    return path
...
def run(codes: list[str], out_dir: Path, cache_dir: Path | None) -> None:
    ...
    write_language_seed(out_dir)
    for code in codes:
        entry = lang_registry.BY_CODE[code]
        _lid, _code, language_qid, name_en, _native = entry
...
def main() -> int:
    ...
    if args.all:
        codes = [row[1] for row in lang_registry.LANGUAGES]
    elif args.language:
        codes = args.language
    else:
